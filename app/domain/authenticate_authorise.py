...
def decode_and_validate_token(user_id: UUID, access_token: str = Depends(oauth2_scheme)):
...
def validate_user_session_from_token(user_id: UUID, session_id: SessionId = Depends(decode_and_validate_token),
...
def authenticate_and_authorise(user_id: UUID, session_id: SessionId = Depends(validate_user_session_from_token)):
    return session_id
